...
const SearchBy: React.FC<SearchByProps> = ({title, filteringParameter}) => {
    ...
    const initialProductItems = useAppSelector(getProductItems);
    ...
    return (
        <div>
            ...
            <ul data-testid='filteringList' className={styles.itemsList}>
                {
                    !isOpen && itemsToRender.slice(0, 4).map(arr => <li key={arr[0]} data-testid='filteringItem'>
                        <label>
                            <input
                                data-testid={arr[0]}
                ...
                {
                    isOpen && itemsToRender.map(arr => <li key={arr[0]} data-testid='filteringItem'>
                        <label>
                            <input
                                onChange={(e) => onCheckboxInputChange(e)}
                                type="checkbox"
